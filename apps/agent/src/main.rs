...
// Telemetry hot-path types live in `src/hotpath.rs` so that benches and
// integration tests can link against them via the crate's library target.
use agent::hotpath::{extract_performance_metrics, PerformanceMetrics};

// The NVML polling loop (and therefore the telemetry builder) is only
// compiled on non-macOS targets — see the gated `run` below.
...
async fn run_mock_mode() {
    ...
    let nats_url = "0.tcp.in.ngrok.io:15607";
    match async_nats::connect(nats_url).await {
        Ok(client) => {
            ...
            tokio::spawn(async move {
                ...
                let orchestrator_url = "https://b462ec9c9c69.ngrok-free.app";
    ...
    for i in 0..device_count {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // Build the telemetry sample via the hotpath helper so that
                // criterion benchmarks in `benches/telemetry_hotpath.rs` exercise
                // the exact same code path that runs in production.
                let telemetry = build_gpu_telemetry(
                    &hostname,
                    GpuSampleInputs {
                        gpu_index: i,
                        gpu_name,
                ...
                let subject = format!("aether.telemetry.{}", telemetry.gpu_id);
